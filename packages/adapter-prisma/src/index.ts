...
export interface PrismaLike {
	[model: string]: {
		...
		upsert: (args: { where: { id: unknown }; create: unknown; update: unknown }) => Promise<unknown>
	}
}
 ...
 */
export function createPrismaPlugin(prisma: PrismaLike, options: PrismaPluginOptions = {}): Plugin {
	const getModel =
		options.modelName ?? ((type: string) => type.charAt(0).toLowerCase() + type.slice(1))
